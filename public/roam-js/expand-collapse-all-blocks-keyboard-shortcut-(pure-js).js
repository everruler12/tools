    ...
    listener = function(e) {
        ...
        if ((e.ctrlKey || e.metaKey) && e.shiftKey && (e.code == 'ArrowUp' || e.keyCode == 38)) {
            ...
            document.querySelector('.page-title')
                .querySelector('button').click() // 3-dot menu next to page title
            // $('.page-title').find('button').click() // 3-dot menu next to page title

            // $('.bp3-portal').find('.bp3-menu').find('div:contains(Collapse all)').click()
            document.querySelectorAll('.bp3-portal')[1] // this is not stable
                .querySelectorAll('.bp3-menu')
                .querySelector('div:contains(Collapse all)') // can't use :contains on elements? https://stackoverflow.com/questions/37098405/javascript-queryselector-find-div-by-innertext/37098508
                .click()
        }
